<template>
  <q-page class="q-pa-md">
    <div class="row items-center justify-between q-mb-md">
      <div class="text-h5">Criar Professor</div>
    ...
    <q-card class="q-pa-md" style="max-width:600px">
      <q-form @submit.prevent="onSubmit" class="q-gutter-md">
        ...
        <div class="row justify-end q-gutter-sm">
          ...
          <q-btn unelevated color="primary" type="submit" :loading="loading" label="Criar Professor" />
        </div>
      </q-form>
    </q-card>
  </q-page>
...
async function onSubmit(){
  ...
  try {
    ...
    const res = await auth.registerProfessor(payload)
    $q.notify({ type:'positive', message:'Professor criado', caption: res.id })
    reset()
  } catch (e) {
    $q.notify({ type:'negative', message:'Erro ao criar professor', caption: e?.message })
  } finally {
    loading.value = false
  }
}
